...
function esm_ap_progress_targets(existingChain, existingMeta) {
    ...
    const eligible = h => {
        const md = get_map_data(h)
        // 目标 = 盟军尚未夺控的图表格(含日控与无主/争夺中的格)。旧实现只认日控
        // (is_space_controlled(h, JP)), 会把第4回合尚无日军进驻的 Buna/Lae/New Georgia
        // 等链首格整段跳过, 只剩 Rabaul/Vogelkop 两个远处日控格, 从而把焦点钉到
        // 不可达的链尾而产生空攻势。这里改为“盟军尚未控制即可作为进程目标”。
        return h >= 0 && h <= LAST_BOARD_HEX && !is_space_controlled(h, AP) && is_controllable_hex(h) &&
    ...
    if (existingChain && existingChain.length && existingChain[0] === 1009) console.error("[APCTL]", (existingChain||[]).map(h=>`${h}:${(get_map_data(h)||{}).name||"?"}:AP=${is_space_controlled(h,AP)}:JP=${is_space_controlled(h,JP)}`).join("  "))
    // 关键修复：候选必须严格按图表链的顺序推进(岛链跳岛：Guadalcanal→Gili-Gili→…
    // →Vogelkop；或 Wake→Tarawa→…→Saipan)。旧实现按“距前线最近”重排，会把链尾
    // 的 Rabaul/Vogelkop(或中太平洋的 Palau) 提到队首——它们同样超出单次攻势可达范围，
    // 反而打乱逐岛推进、使每一步都跳向不可达目标而产生空攻势。这里只保留图表顺序。
